...
func main() {
	...
	cred, err := createLinkedDomainCredential(issuerDID, verificationMethod, domain)
	exitOnErr("create credential", err)

	output := &didconfiguration.DidConfiguration{
		Context:    didConfigurationContextURL,
		LinkedDiDs: []interface{}{cred.JWTEnvelope.JWT},
	}
...
func createLinkedDomainCredential(issuerDID, verificationMethod, domain string) (*verifiable.Credential, error) {
	...
	keyID, _, err := km.CreateJWKKey(kmsapi.ED25519Type)
	if err != nil {
		return nil, err
	}
	...
	return cryptoSvc.SignCredential(signer, unsignedVC)
}
